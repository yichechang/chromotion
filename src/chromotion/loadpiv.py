...
def read_matpiv(matfpath) -> xr.Dataset:
    ...
    ds = (
        xr.concat(
            [convert_single_lag(d) for d in pivresult["data"] if d["piv"] is not None],
            dim="lag",
        )
        .sortby("lag")
    ...
    ds = ds.assign_coords(
        {
            "X_um": ("X", ds["X"].data * ds.attrs["pixelsize"]),
            "Y_um": ("Y", ds["Y"].data * ds.attrs["pixelsize"]),
            "T_s": ("T", ds["T"].data * ds.attrs["dt"]),
            "lag_s": ("lag", ds["lag"].data * ds.attrs["dt"]),
        }
    )
...
def convert_pivresult_to_dict(matfpath) -> dict:
    ...
    mat = scipy.io.loadmat(matfpath, struct_as_record=True)
...
def convert_single_lag(data) -> xr.Dataset:
    ds = (
        xr.concat([convert_single_piv(d) for d in data["piv"]], dim="T")
        .sortby("T")
...
def struct_to_dict(struct, vec1d=False) -> dict:
    ...
    if _is_struct_array(struct) and not _has_only_one_struct(struct):
        raise ValueError("Multiple structs instead of one are passed.")
