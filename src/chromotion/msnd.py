...
class MSND:
    ...
    def __init__(
        ...
        if isinstance(data, xr.Dataset):
            if components is None:
                raise ValueError("components must be specified if data is a Dataset.")
    ...
    def _use_pixel_as_unit(self):
        self._mpp = 1.0
        print("Cannot find physical pixel size. Unit for MSND is now in pixel squared.")

    def calculate(
        self, method: str = "normal", **kwargs
        ...
        **kwargs
        ...
        df = da.to_dataframe().dropna(axis=0, how="any").reset_index()[self._colnames]
        ...
        if self._fps is not None:
            ...
            print(
                f"Overwriting lag_s (and T_s) using fps = {self._fps} "
    ...
    def _weighted_msnd(self, weights: xr.DataArray) -> xr.DataArray:
    ...
    def _eachlevel_msnd(
        ...
        D2, image = xr.align(
            self._D2.rename("D2"), byimage.rename("image"), join="left"
        )
        ...
        for i, b in enumerate(binlabels):
            msnd.append(self._msnd(D2.where(digitized == i)).assign_coords(level=b))

        return xr.concat(msnd, dim="level")

    def _eachlevel2d_msnd(
        self,
        byimage: xr.Dataset,
        bins: np.ndarray,
    ) -> xr.DataArray:
        if isinstance(bins, int):
            raise ValueError(
                f"2D-level MSND only supports `bins` "
...
def plot_msnd_stats(df, ax=None, errorbar=True, **kwargs):
    ...
    if errorbar:
        _plot_msnd_stats(
            ax, df["lag_s"], df["mean"], df["std"] / np.sqrt(df["size"]), **kwargs
        )
    else:
...
def plot_msnd_raw(df):
    g = sns.relplot(
        data=df, x="T_s", y="msnd", hue="lag_s", kind="line", palette="Set2"
    )
    return g


def calculate_msnd(
    ds, components=["u", "v"], weight=None, pixsize=None
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    df = piv[["msnd"]].to_dataframe().dropna(axis=0, how="any").reset_index()
